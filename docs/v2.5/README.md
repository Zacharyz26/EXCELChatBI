...
> 状态：v2.5 阶段 3–6 已通过真实 Compose CI 工程关闭
> 更新日期：2026-08-14
...
| 6：自主分析 | 数据角色、结果驱动分析、多数据集、受控并行与统计护栏 | 6A～6E 已完成并通过远程完整 CI |
...
6B-2 已把角色确认绑定 TaskRun 计划/数据版本，并在统计/聚合执行前接入确定性门禁。
6B-3 已冻结 16 个匿名场景/65 列的代表性评测集，并接入 `data-role-quality`
CI 强制门禁与脱敏产物。6B-4 已接入生产处理器、stdio、Streamable HTTP 三路目录/结果
等价及真实 `data-tools` 重启恢复门禁；提交 `6b89ef6` 的
[run 31467721161](https://github.com/Zacharyz26/EXCELChatBI/actions/runs/31467721161)
已确认 backend、frontend 与真实 Compose 三项全绿并关闭 6B。随后
[阶段6C](./阶段6C实施记录.md)：6C-1 已本地实现有界候选假设、确定性角色/capability 筛选、
TaskRun 审计持久化和 React 人工选择入口；6C-2 已绑定 Plan/Invocation/Evidence Ledger/
Verifier 状态并在 API 与 React 展示；6C-3 已实现受预算、重规划和取消边界约束的确定性
停止/降级/补证/下一候选投影，且后续动作必须经用户确认创建分支；6C-4 已加入 14 场景匿名
...
已确认 backend、frontend 与真实 Compose 三项全绿并关闭 6C。
[阶段6D](./阶段6D实施记录.md) 已由提交 `3febd68` 的
[run 31678576324](https://github.com/Zacharyz26/EXCELChatBI/actions/runs/31678576324)
完整 CI 关闭。[阶段6E](./阶段6E实施记录.md) 的 6E-1 已实现只读 Join 预检，
6E-2 已实现高风险参数绑定审批、固定 Join 执行、SQLite v11 多父血缘、TaskRun 双父版本
绑定和派生策略继承；6E-3 已实现精确预检/数据版本门禁、React 审批恢复与完整双父
血缘展示；6E-4 已实现 17 场景脱敏质量门禁、跨项目/敏感键/高风险授权发布契约、
stdio/HTTP 等价、`data-tools` 重启恢复和浏览器验收。提交 `92a6f02` 的
[run 31767613363](https://github.com/Zacharyz26/EXCELChatBI/actions/runs/31767613363)
已确认 backend、frontend 与 containers/Compose 三项全绿，阶段 6 工程关闭。
...
- [`./阶段6C实施记录.md`](./阶段6C实施记录.md)：受控候选假设、结果驱动探索与当前边界。
- [`./阶段6D实施记录.md`](./阶段6D实施记录.md)：统计 Evidence、预测护栏与当前边界。
- [`./阶段6E实施记录.md`](./阶段6E实施记录.md)：多数据集 Join 预检、授权与后续版本治理。
