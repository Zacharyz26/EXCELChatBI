...
> 状态：阶段 3A 实现已收口；待本批提交后的 Docker runner 恢复门禁确认
> 更新日期：2026-07-30
...
当前代码与验证状态见
[`阶段3A实施记录.md`](./阶段3A实施记录.md)。3A-1–3A-4 均已落地，包含离线一致
备份/恢复、readiness fail-closed 和 Compose 联合恢复门禁；由于本机没有 Docker，
阶段关闭仍需本批提交后的 Docker runner 实际执行结果。

后续按以下顺序推进：

1. **3A 契约与持久化**：实现已收口，等待 Docker runner 发布门禁确认；
2. **3B 上下文压缩**：实现 `compaction.py`，区分最近原文和滚动摘要；
3. **3C 指代消解**：实现 `coref.py`，解析 Artifact、Dataset 和已确认实体引用；
