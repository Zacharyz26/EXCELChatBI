...
### 10.5 阶段 2 结构化计划行为对照（2026-07-28）

`scripts/stage2_behavior_eval.py` 对冻结的 20 个场景运行 3 次，只隔离
`deepseek-v4-flash`，并开启生产 `TaskPlan → Executor → Replanner → deterministic
Verifier` 真路径，共 60 runs：

| 任务成功 | Artifact 交付 | 数值有据 | 终态如实 | 澄清准确 | 越界违反 | 无效调用 | 成本(USD) |
|---:|---:|---:|---:|---:|---:|---:|---:|
| **70.0%** | 74.1% | 93.3% | **73.3%** | 95.0% | **0** | 73 | 0.095 |

任务成功率严格高于 Flash 基线 26.7%，终态如实率严格高于 36.7%，实际模型、60 行、
场景 hash、计费可用性和隐私契约均匹配，自动裁决为 `PASS`。报告见
`.../stage2/report.json`。

本结果同时暴露出明显债务：计划完成率仅 68.9%；18/60 未满足任务；5 个场景三次均失败；
一个多步骤场景贡献 52 次无效调用，两个 run 各达到 33 次模型轮次。失败主要集中在终态
分类、失败恢复、follow-up 上下文、Required Artifact 和数值 Claim 支持，不能因为相对
基线通过而忽略。首轮还曾因单个流式 `ReadTimeout` 触发旧 harness 整批取消，因此评测入口
已增加逐 run 原子检查点、异常隔离和协议校验续跑。

### 10.5.1 首轮失败分析后的 P0 修复

2026-07-29 已按失败分布落地三个 P0，但冻结报告保持原值：

- Executor 以全部工具尝试而非仅真实执行数消耗预算；稳定签名不再包含随机 call ID，
  并由无效调用上限和模型轮次上限保证终态收敛；
- Planner 使用安全 Artifact 摘要判断已有结果是否满足 follow-up 前置能力，避免为报告
  重跑趋势/图表，也避免把图表展示粒度修改误判为新的趋势分析；
- Host 依据 TaskContract 和当前项目 Artifact 血缘补齐 PDF、analysis_ids 和 chart
  dataset_ref，显式非法或跨项目 dataset_ref 仍交给 Policy 拒绝，不被静默覆盖。

验收必须使用新增、未见的 follow-up/Artifact/无效调用场景重新冻结评测集并至少重复三次；
已查看的 B05/B11/B14/B15 只能作为回归，不得继续作为改进分数。复评至少报告任务成功率、
终态如实率、Artifact 交付率、计划完成率、无效调用数、模型轮次分布和越界违反。

### 10.6 提议冻结的验收门槛（待人工评审签字）

基线已产生，可据实测分布提议门槛；**最终数值由评审冻结（item 6，尚未完成），本表为提议值**：
`scripts/g7_review_gate.py` 已把这些提议值实现为拒绝式签字门禁；人工评分或签字缺失时只会
输出 `review_required`。阶段 2A–2E 已复用本文件的生产路由规则；
`scripts/stage2_behavior_eval.py` 已完成开启结构化计划控制面的 20×3 Flash 对照，
并以 `--confirm-paid-run` 防止误触发真实模型费用。阶段 2 报告缺失、协议不完整或指标未
严格超过基线时，G7 同样拒绝签字；本次自动门禁已通过，但不能替代人工冻结门槛。
