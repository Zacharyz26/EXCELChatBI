...
> 状态：双传输技术决策与最新 Compose/容器 CI 已验证；G7 自动门禁已落地，
> ADR 仍待代表性人工评测和评审接受 · 更新日期：2026-07-29
...
五服务独立路由、逐服务认证、私网 Compose、分卷和浏览器/重启门禁；最新 Docker runner
已全绿。ADR 的“接受”状态仍须等待代表性 G7 人工评测和设计评审。

G7 的冻结门槛、缺失人工评分和签字命令见
[`G7评审与阶段2A实施记录.md`](./G7评审与阶段2A实施记录.md)。自动门禁通过不能代替
Planner/Verifier 匿名盲评和负责人签字。
