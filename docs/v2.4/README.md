...
> 状态：阶段 1 已验收通过；阶段 0 G1–G6 实测完成，G7 仅剩人工盲评/评审签字与 ADR 接受（item 6） · 更新日期：2026-07-23
> 范围：设计依据、阶段 0 未关闭门禁和阶段 1 实施状态；本版本不新增分析工具

本目录把 `docs/Agent自主化开发规划.md` 中的 v2.4 展开为实现依据。当前生产基线仍是
v2.3；阶段 1 已在阶段 0 门禁未全部关闭时先行收尾，并在推送后由 GitHub Actions run #5
（commit 32e6685）关闭镜像门禁、正式验收通过；阶段 0 未关闭项继续作为进入阶段 2 的显式
前置债务。只有 [`阶段1实施记录.md`](./阶段1实施记录.md) 标记“已实现”的条目可以写成交付
事实，其余设计仍不得提前宣称完成。
...
| [阶段 0 继续计划](./阶段0继续计划.md) | 未关闭门禁的执行方法、顺序、产物与 go/no-go | 执行中（G3/G5/G6 已完成） |
...
以及据评审把 ADR 从“草案”改“接受”并正式冻结软门槛。在此之前 ADR 保持“草案”，阶段 0 不算
正式关闭，阶段 2 不启动。执行方法与顺序见 [`阶段0继续计划.md`](./阶段0继续计划.md)。
