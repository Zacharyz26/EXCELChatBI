...
> 版本：v2.5-stage6c-in-progress · 状态：v2.4 阶段 2A–2E 与 v2.5 阶段 3–5、6A/6B 工程关闭，
> 提交 `0b5980c` 的完整 CI/Docker 恢复门禁全绿；G7 代表性评测/人工签字待完成；
> 提交 `d5a672d` 的阶段 5 及提交 `b67b704` 的阶段 6A Compose 工程门禁全绿；
> 阶段 6C-1～6C-4 候选筛选、结果生命周期、确定性跟进与发布门禁本地实现完成，待完整 CI 签字
> · 语言场景：中文优先
...
> **当前状态**：v2.3 已将前端从五个菜单功能页重构为对话式工作区并下线经典页面。
> v2.5 阶段 4B 已增加真实 TaskRun/计划、结构化澄清、暂停恢复和审批交互；证据/工具
> 健康完整视图、重连恢复与三档自主等级已由 4C/4D 实现；阶段 4 已由提交 `7f81fe2`
> 的真实 Compose CI 关闭。
...
3. **v2.5 记忆、自主性与协作 — 阶段 3A–3E 已完成**：领域无关的 schema v4、
   ...
   工程门禁全绿。阶段 6A 也已由提交 `b67b704` 的完整 CI 关闭：schema v9 固定 TaskRun
   capability/tool 目录并完成受治理
   换代/profile 可用性，schema v10 建立共享预算、固定数据版本、取消树和 Evidence Ledger；
   6B-1 已补确定性数据角色、置信/歧义、只读质量建议及前端展示；6B-2 已补绑定 TaskRun
   计划/数据版本的结构化角色确认与统计/聚合前置门禁；6B-3 已冻结匿名代表性评测并接入 CI，
   6B-4 已接入生产处理器/stdio/HTTP 等价与真实 `data-tools` Compose 重启恢复门禁；提交
   `6b89ef6` 的完整 CI 全绿并关闭 6B。6C-1 已实现有界 `tested=false` 候选、确定性角色/
   capability 筛选、TaskRun 审计持久化及 React 选择入口；6C-2 已把选中候选绑定不可变计划、
   Invocation、Evidence Ledger 与 Verifier 终态；6C-3 已加入确定性停止、降级、补证和下一
   候选投影，后续动作禁止自动执行并受预算、重规划、取消树和人工分支确认约束；6C-4 已加入
   14 场景匿名评测、CI 强制门禁和 Compose 浏览器/三次恢复探针，等待完整 CI 对真实 Compose
   签字后关闭。后续继续统计/预测护栏和多数据集。
...
4B 已完成 React TaskRun、计划、结构化澄清、任务控制和审批交互，详见
`docs/v2.5/阶段4B实施记录.md`；4C 服务端/SSE 恢复、工具审计和 Compose 重启门禁已由
...
阶段 6A 的 TaskRun capability 目录冻结、受控并行和 CI 关闭证据见
`docs/v2.5/阶段6A实施记录.md`。阶段 5–6 在真实
