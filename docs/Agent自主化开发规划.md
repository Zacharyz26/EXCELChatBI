...
> 场景和 Verifier 评分契约后再关闭 G7。v2.5 阶段 3A 实现已收口，包含一致备份、
> readiness 和 Compose 联合恢复门禁；阶段关闭等待本批提交后的 Docker runner 结果。
...
> 当前实施入口：[`v2.5/阶段3A实施计划.md`](./v2.5/阶段3A实施计划.md)。先实现领域无关的
> 记忆契约、持久化、Memory Policy、不可变快照和项目隔离；不因真实业务需求尚未明确而
> 预置销售/利润等领域记忆。
