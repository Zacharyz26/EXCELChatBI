...
> 基线：v2.4 阶段 1 已验收，阶段 2A–2E 已实现；v2.5 阶段 3–5、6A～6D 工程关闭，
> 当前开发入口为阶段 6E 收尾；6E-1～6E-4 已本地实现，待远程完整 CI
...
> 完整 CI 关闭。当前 6E-1 Join 只读预检、6E-2 固定 Join 执行/参数绑定审批/SQLite v11
> 多父血缘/派生策略继承、6E-3 精确版本门禁/React 审批恢复/双父血缘展示，以及 6E-4
> 17 场景脱敏评测、跨项目/敏感键/授权契约、双传输、Compose 恢复和浏览器门禁均已本地实现；
> 下一步为提交后验证远程 backend/frontend/containers 完整 CI，通过后关闭 6E。
...
> 完整 CI 关闭，详见 [`v2.5/阶段6D实施记录.md`](./v2.5/阶段6D实施记录.md)。当前进入
> [`v2.5/阶段6E实施记录.md`](./v2.5/阶段6E实施记录.md) 的多数据集关联与版本治理。
