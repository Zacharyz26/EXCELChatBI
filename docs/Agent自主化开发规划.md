...
> 状态：当前开发依据 · 制定日期：2026-07-21 · 更新日期：2026-08-14
> 基线：v2.4 阶段 1 已验收，阶段 2A–2E 已实现；v2.5 阶段 3–6 已工程关闭
> 当前没有未完成的 v2.5 功能阶段；后续需从独立发布债务、安全项目或 v3.0 路线单独立项
...
> SSE harness 和报告卡片定位问题已修复，最新 Docker runner 全绿；
> 阶段 2 的 20×3 真实行为对照已完成并通过自动门禁（成功 70.0%、终态如实 73.3%、
> 越界 0）。现有场景全部使用商业数据语境，人工评分暂缓；保留其回归价值，补充代表性
> 场景和 Verifier 评分契约后再关闭 G7。v2.5 阶段 3A 已完成；提交 `162b170`
> 的完整 CI（含 Compose 联合恢复门禁）全绿。3B-1–3B-3 与 3C-1–3C-3 已由
> 提交 `e3d51fd` 的真实 Docker/Compose CI 验证并关闭。3D-1–3D-3 用户记忆治理与
> 3E-1–3E-3 的 SQLite v6 锚点、五阶段来源图、安全查看、领域中立质量和联合恢复探针
> 已由提交 `0b5980c` 的真实 Docker/Compose CI 验证并关闭。v2.5 阶段 3 已全部完成，
> 阶段 4A-1/4A-2 已完成 SQLite v7、用户计划修订、ApprovalRecord 后端契约及
...
> 选中候选绑定不可变计划、Invocation、Evidence Ledger 和 Verifier 终态；6C-3 已实现由
> Observation/Verifier、共享预算、重规划上限与取消树约束的确定性结果跟进；6C-4 已加入
> 14 场景匿名确定性评测、CI 强制门禁和 Compose 浏览器/三次恢复探针；提交 `d5005ee` 的
...
> 完整 CI 关闭。6E-1 Join 只读预检、6E-2 固定 Join 执行/参数绑定审批/SQLite v11
> 多父血缘/派生策略继承、6E-3 精确版本门禁/React 审批恢复/双父血缘展示，以及 6E-4
> 17 场景脱敏评测、跨项目/敏感键/授权契约、双传输、Compose 恢复和浏览器门禁均已实现；
> 提交 `92a6f02` 的
> [CI run 31767613363](https://github.com/Zacharyz26/EXCELChatBI/actions/runs/31767613363)
> 已确认 backend、frontend 与真实 Compose 三项全绿，阶段 6E 及 v2.5 阶段 6 工程关闭。
...
- 基础中央策略、结构化审计和模型/工具 trace 已落地；确定性持久化上下文压缩、
  领域中立质量门禁和 Compose 固定版本恢复探针已接入；Artifact/Dataset 确定性
  指代解析、歧义失败关闭、计划绑定、固定快照实体映射及 3C 质量/恢复门禁已完成并通过
  实际容器 CI；3D 用户治理和 3E 完整血缘/恢复也已通过真实容器 CI；4A-1/4A-2 已落地
...
> runner 已全绿。v2.4 代码/容器工程门禁已关闭，G7 代表性评测、人工签字和 ADR
> 接受继续作为显式发布债务。
...
> 当前状态：v2.5 阶段 3–6 已由各阶段的真实 Compose CI 工程关闭；本节作为已交付记录保留。
...
- 摘要只用于上下文导航，不得成为数值证据；
- 完成 Dataset→Analysis/Invocation→Artifact→Evidence→Claim 的完整来源图；SQLite v6、
  安全 API/React、领域中立质量和恢复探针已实现并通过真实 Docker/Compose CI。
...
> 4A-1/4A-2 实施结果见 [`v2.5/阶段4A实施记录.md`](./v2.5/阶段4A实施记录.md)：
> SQLite v7、暂停边界计划修订、ApprovalRecord 原语、高风险调用前暂停、批准恢复消费及
> Gateway/Server 双重校验已完成。4B 实施结果见
> [`v2.5/阶段4B实施记录.md`](./v2.5/阶段4B实施记录.md)：React TaskRun、计划、结构化
> 澄清、任务控制和审批交互已完成。4C 功能与验证边界见
> [`v2.5/阶段4C实施记录.md`](./v2.5/阶段4C实施记录.md)：SSE/服务端恢复与工具审计已由
> `2f2771f` 的真实 Compose CI 验证关闭。4D 见
> [`v2.5/阶段4D实施记录.md`](./v2.5/阶段4D实施记录.md)：三档自主等级、终态分支对比和
> 追加式反馈已实现；扩展的辅助/反馈/只读 Compose 场景已由提交 `7f81fe2` 的真实 CI
> 验证关闭，阶段 4 完成。
...
> 5A 版本化定义、受控公式和 Evidence 工程切片已完成；5B-1 已实现领域定义 Resource
...
领域语义必须先于大规模自主探索；业务指标只是可能的实例，不能在真实需求未知时预置为
唯一目标领域：

- 建立领域定义及可计算指标的公式、粒度、适用范围、负责人、版本和生效日期；
- 字段与知识库概念映射；
- 口径冲突、过期和缺失检测；
...
验收：现有商业回归中的“按公司定义计算复购率”只作为版本化定义示例；真实验收使用目标
领域的代表性定义。系统必须先解析可执行定义再计算，冲突时进入澄清，不由模型自行选择；
旧报告仍能定位旧定义版本；索引切代、容器重建和 CPU/GPU 切换不破坏 Resource、Claim
和拒答语义。
...
> Compose 重启恢复门禁全绿。6C 受控候选假设与结果驱动探索也已由提交 `d5005ee` 的
> [run 31659188951](https://github.com/Zacharyz26/EXCELChatBI/actions/runs/31659188951)
> 完整 CI 关闭，详见 [`v2.5/阶段6C实施记录.md`](./v2.5/阶段6C实施记录.md)。6D 也已由
> 完整 CI 关闭，详见 [`v2.5/阶段6D实施记录.md`](./v2.5/阶段6D实施记录.md)。
> [`v2.5/阶段6E实施记录.md`](./v2.5/阶段6E实施记录.md) 也已由提交 `92a6f02` 的完整 CI 关闭。
...
完整记忆、可干预前端、领域语义层、自主探索、多数据集分析和统计护栏；同时补齐对应的 MCP 记忆/知识引用、工具来源展示以及单机容器资源与恢复能力。
