...
MVP 主链路已闭环，前后端在浏览器跑通。当前阶段：**对话式 Agent 产品重构**（v2.2 定案，设计文档第 14 章）——阶段 0/1/2/3 已完成：模型网关地基、项目/历史会话持久化与对话工作区、Agent 工具注册表（11 个工具，schema 同源）、Agent 循环（/chat/stream 即对话式 Agent：自动规划调工具 + SSE 透明度卡片 + 分析登记表追问 + 快捷指令条）；下一步进入阶段 4 迁移收尾。
