...
MVP 主链路已闭环，前后端在浏览器跑通。当前阶段：**对话式 Agent 产品重构**（v2.2 定案，设计文档第 14 章）——阶段 0/1 已完成，已具备模型网关地基、项目/历史会话持久化、Excel 画像卡和纯 LLM SSE 流式对话；下一步进入阶段 2 工具封装。
...
- **下一步**阶段 2 工具封装：现有能力注册为 Agent tools + 新增结构化 transform/aggregate 工具 + 衍生数据集血缘 + report 组装式重构。
