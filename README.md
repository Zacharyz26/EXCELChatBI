...
MVP 主链路已闭环，前后端在浏览器跑通。当前阶段：**对话式 Agent 产品重构**（v2.2 定案，设计文档第 14 章）——从菜单式分析工具升级为对话驱动的数据分析 Agent，五阶段迁移，严格按阶段推进。
...
- 阶段 1 对话工作区：SQLite 持久层（项目/对话/消息/工件）+ 侧边栏/消息流/输入框新前端（zustand）。
- 阶段 2 工具封装：现有能力注册为 Agent tools + 新增结构化 transform/aggregate 工具 + 衍生数据集血缘 + report 组装式重构。
- 阶段 3 Agent 循环：自动规划 → 调工具 → SSE 透明度事件（理解/计划/执行卡）→ 追问关联。
