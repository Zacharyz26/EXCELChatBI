...
from packages.session.migrations import v2, v3, v4, v5, v6, v7, v8, v9

CURRENT_SCHEMA_VERSION = v9.VERSION
...
def migrate_database(
    ...
    *,
    ...
    if version == 0:
        ...
        _apply_v7(connection, source_version=6, backup=None)
    ...
    if version == v6.VERSION:
        ...
        backup = _backup_database(connection, db_path, "v6-backup")
        _apply_v7(connection, source_version=6, backup=backup)
        _apply_v8(connection, source_version=7, backup=None)
        _apply_v9(connection, source_version=8, backup=None)
        return
    if version == v7.VERSION:
        ...
        backup = _backup_database(connection, db_path, "v7-backup")
...
def downgrade_v2_to_v1(db_path: str | Path) -> Path:
    ...
    with _connect(path) as connection:
        ...
        if version not in {
            ...
            raise RuntimeError(
                f"只能从 schema v2/v3/v4/v5/v6/v7/v8/v9 回滚，当前版本为 {version}"
            )
...
def _apply_v7(
    ...
    *,
    ...
    try:
        ...
        connection.execute(f"PRAGMA user_version = {v7.VERSION}")
        connection.commit()
    except Exception:
        connection.rollback()
        raise
