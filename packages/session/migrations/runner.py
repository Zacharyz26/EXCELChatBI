...
from packages.session.migrations import v2, v3

CURRENT_SCHEMA_VERSION = v3.VERSION
...
def downgrade_v2_to_v1(db_path: str | Path) -> Path:
    ...
    with _connect(path) as connection:
        ...
        if version not in {v2.VERSION, v3.VERSION}:
            raise RuntimeError(f"只能从 schema v2/v3 回滚，当前版本为 {version}")
