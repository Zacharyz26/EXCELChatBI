...
PROMPT_VERSION = "semantic-verifier-v3"
...
- 判断用户是否已得到答案时只看最终 Claim 明确写出的内容；Evidence 只能证明 Claim，
  不能替最终答复补上遗漏的地区、时间范围、方法、口径、比较对象或局限。
- criterion=pass 必须引用至少一个直接覆盖该标准的 claim_id；不能只因 Evidence 含有信息就 pass。
- Claim 仅提出澄清问题时表示任务仍在等待用户，不是完成；使用 uncertain + clarify。
- 必须披露的方法、假设和局限要出现在 Claim.text 或 Claim.limitations 中；
  只出现在 Evidence 或 assumptions 中仍视为未披露。
...
fail 或 uncertain 可以使用空 claim_ids/evidence_ids。全部通过且无其他问题时
...
class SemanticVerifierProtocolError(RuntimeError):
    ...
    def __init__(
        ...
        *,
        ...
        self.pricing_effective_date = pricing_effective_date
        ...
        *,
        ...
        try:
            ...
            raise SemanticVerifierProtocolError(
                ...
                pricing_effective_date=response.pricing_effective_date,
            ) from exc
        return SemanticEvaluation(
...
def _parse_response(content: str, request: SemanticVerificationRequest) -> JsonObject:
    ...
    for item in raw_criteria:
        cited_claims = {str(value) for value in cast(list[str], item["claim_ids"])}
        ...
        if item["status"] == "pass" and (not cited_claims or not cited):
            raise SemanticVerifierProtocolError(
                "语义 criterion=pass 必须同时引用 Claim 与 Evidence"
            )
...
def _derive_verdict(
    ...
    decisions = tuple(
        SemanticCriterionDecision(
            ...
            claim_ids=tuple(str(value) for value in cast(list[str], item["claim_ids"])),
