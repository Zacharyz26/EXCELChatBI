...
# 让 DeepSeek 只吐 JSON（OpenAI 兼容 json_object 模式；prompt 中已含"JSON"字样）。
_JSON_MODE: dict[str, object] = {"response_format": {"type": "json_object"}}


def build_messages(profile: DataProfile, feedback: str | None = None) -> list[Message]:
    """构造发往模型的消息：system + 仅含画像的 user（+ 可选纠错反馈）。"""
    payload = profile.to_dict()
    # 红线1 可观测：明确记录"只发画像"，便于审计/演示验证（重规划时同样成立）
    _log.info(
        "planner.payload",
    ...
    if feedback:
        # 带错重规划：把上一次的失败原因回传，约束模型改用合法列名/枚举。
        valid_cols = "、".join(c["name"] for c in payload["columns"])
        messages.append(
            Message(
                role="user",
                content=(
                    f"上一次规划有误：{feedback}\n"
                    f"可用列名仅限：{valid_cols}。chart_type 仅限 line/bar/pie/scatter，"
                    f"agg 仅限 sum/mean/count/none。请重新只输出严格 JSON。"
                ),
            )
        )
    return messages
...
async def plan_chart(
    profile: DataProfile, gateway: ModelGateway, feedback: str | None = None
) -> dict[str, Any]:
    """调用模型规划图表，返回可直接喂给 gen_chart 的入参。

    Args:
        profile: 数据画像（红线1：模型只见画像）。
        gateway: 模型网关。
        feedback: 上一次失败原因；提供则触发带错重规划。

    Returns:
        {dataset_ref, chart_type, encoding:{x, y, agg}}。
    """
    messages = build_messages(profile, feedback)
    resp = await gateway.complete(Scenario.CORE_REASONING, messages, params=_JSON_MODE)
