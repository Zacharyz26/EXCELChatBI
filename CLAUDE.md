...
当前生产基线是 **v2.4 阶段 2E + 已完成的 v2.5 阶段 3A–3E + 开发中的 4A**。3A–3C 的历史
...
| 对话持久层 | SQLite v7 `.data/chatbi.db` + LRU 热缓存；Task/Event/Plan/Step/Evidence/Claim/Checkpoint/MemorySnapshot/ConversationCompaction/LineageAnchor/ApprovalRecord | 阶段 3 已关闭；4A-1/4A-2 后端协作与审批执行链已完成 |
...
### 6.2 当前任务：v2.5 阶段 4A 协作契约与 G7 评审债务
...
4. 阶段 4A-1/4A-2 已完成 SQLite v7、暂停边界的不可变计划修订、ApprovalRecord
   ...
   Gateway/Server 双重校验。下一入口为 4B React 协作界面；结构化澄清、恢复交互和
   自主等级仍属于阶段 4，不得把前端按钮当作授权。

完整状态见 `/docs/v2.5/README.md`。

### 6.3 已纳入未来版本，不再视为永久禁区

- v2.5：阶段 3 已关闭、阶段 4A 开发中；继续可干预前端、业务语义层、
  自主分析和多数据集；
